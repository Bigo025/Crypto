...
class Relay:
    ...
    def listenBlockchain(self, server):
        ...
        server._app.route("/Blockchain", method="GET",
                          callback=self.getBlockchain)

    def listenDifficultyRequest(self,server):
        server._app.route("/DifficultyRequest", method="GET",
                          callback=self.getDifficulty)

    def listenUnspentTx(self,server):  # ecoute le port 
    ...
    def listenGetInitialTransaction(self, server):
        ...
        server._app.route("/InitialTransaction", method="GET",
                          callback=self.getInitialTransactionFromWallet)

    def listenBlockToMaster(self, server):
        """Listen for transfering a block to the master."""
        server._app.route("/BlockToMaster", method="POST",
                          callback=self.transferBlock)

    def listenTxFromWallet(self, server):
        """Listen for adding a transaction to the pool of txs."""
        server._app.route("/TxToRelay", method="POST",
                          callback=self.addTxToPool)
    ...
    def listenBalanceRequest(self, server):
        ...
        server._app.route("/BalanceRequest", method="POST",
                          callback=self.getBalance)
    ...
    def getUnspentTx(self): # retourne la liste des transactions du Master 
